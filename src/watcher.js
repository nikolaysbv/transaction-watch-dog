...
class Watcher {
  ...
  handleMessage() {
    ...
    return async (data) => {
      ...
      if (message.id === 1) {
        // returns on the subscription successful message
        return;
      }
      ...
      for (let transaction of transactions) {
        ...
        for (let configuration of configurations) {
          ...
          const applicableRules = Object.entries(
            configuration.dataValues
          ).filter((rule) => {
          ...
          for (let rule of applicableRules) {
            ...
            if (
              ...
              const [comparisonOperator, comparisonNumber] = value.split(
                " ",
                2
              );
              const rulePassed =
